...
//	HAL_TIM_Base_Start_IT(&htim7);  // <<< Quan trọng: bật ngắt mỗi chu kỳ PWM
...
int main(void) {
	...
	osThreadDef(Task_Read_Encod, Run_Task_Read_Encoder, osPriorityNormal, 0,
			128);
...
/* USER CODE BEGIN Header_Run_Task_Read_Encoder */
/**
 * @brief  Function implementing the Task_Read_Encod thread.
 * @param  argument: Not used
 * @retval None
 */
/* USER CODE END Header_Run_Task_Read_Encoder */
void Run_Task_Read_Encoder(void const *argument) {
	/* USER CODE BEGIN 5 */
	/* Infinite loop */
	for (;;) {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	...
	if (htim->Instance == TIM7) {
		encoder_update(&encoder_value);
//		__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, pulse / 2);
//		__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_2, pulse / 3);
//		__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_3, pulse / 4);

		// Set giá trị cố định để test
//	    TIM8->CCR1 = pulse / 2;
//	    TIM8->CCR2 = pulse / 3;
//	    TIM8->CCR3 = pulse / 4;
	}
